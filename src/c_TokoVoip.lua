...
function TokoVoip.updateTokoVoipInfo(self, forceUpdate) -- Update the top-left info
	...
	if (self.plugin_data.phoneChannel ~= nil and self.plugin_data.phoneChannel ~= -1) then
		info = info .. "<br> [Telefono] " .. self.plugin_data.phoneChannel
	end
...
function TokoVoip.initialize(self)
	...
	Citizen.CreateThread(function()
		while (true) do
			...
			-- swiching radio channel and change
			-- voicemod proximity
			...
			elseif (IsControlJustPressed(0, self.keyProximity)) then -- Switch proximity modes (normal / whisper / shout)
				-- questo dovrebbe essere apposto visto che controlla
				-- quando premi il tasto per cambiare
				if exports.chat:getStopChat() then
				   return
				end
			...
			if (IsControlPressed(0, self.radioKey) and self.plugin_data.radioChannel ~= -1 and self.config.radioEnabled) then -- Talk on radio
				...
				if (not lastTalkState and self.myChannels[self.plugin_data.radioChannel] and self.config.radioAnim) then
					if (not string.match(self.myChannels[self.plugin_data.radioChannel].name, "Call") and not IsPedSittingInAnyVehicle(PlayerPedId())) then
						...
						while not HasAnimDictLoaded("random@arrests") do Citizen.Wait(5) end
						
						TaskPlayAnim(GetPlayerPed(-1), "random@arrests", "generic_radio_chatter", 8.0, 0.0, -1, 49, 0, 0, 0, 0)

						Citizen.CreateThreadNow(function()
							while lastTalkState do
								if not IsEntityPlayingAnim(GetPlayerPed(-1), "random@arrests", "generic_radio_chatter", 49) then
									TaskPlayAnim(GetPlayerPed(-1), "random@arrests", "generic_radio_chatter", 8.0, 0.0, -1, 49, 0, 0, 0, 0)
								end
								Citizen.Wait(500)
							end
					...
					TriggerEvent("InteractSound_CL:PlayOnOne", "01-on", 0.4)
					lastTalkState = true
				end
			else
				...
				if lastTalkState and self.config.radioAnim then
					lastTalkState = false
					StopAnimTask(PlayerPedId(), "random@arrests", "generic_radio_chatter", -4.0)
					TriggerEvent("InteractSound_CL:PlayOnOne", "01-off", 0.4)
				end
			end

			-- controllo talk telefono
			-- if self.plugin_data.phoneChannel ~= nil and self.plugin_data.phoneChannel ~= -1 then
			-- 	self:updateTokoVoipInfo();
			-- end
		end
