...
local function clientProcessing()
	...
	for _, channel in pairs(voip.myChannels) do
		for _, subscriber in pairs(channel.subscribers) do
			...
			if ((type(remotePlayerChannel) == "number" and remotePlayerChannel <= voip.config.radioClickMaxChannel) or channel.radio) then
				...
				-- TriggerEvent("InteractSound_CL:PlayOnOne", "01-on", 0.6)
			end
	...
	-- process phone calls
	for _, channel in pairs(voip.phoneChannels) do
		for _, subscriber in pairs(channel.subscribers) do
			if (subscriber == voip.serverId) then goto phoneChannelContinue end

			-- controllo se il giocatore si trova nello stesso canale
			-- di chi sta facendo cose
			local remoteUUID = getPlayerData(subscriber, "voip:pluginUUID");
			local remotePlayerPhoneChannel = getPlayerData(subscriber, "phone:channel");
			if remotePlayerPhoneChannel ~= channel.id then goto phoneChannelContinue end
			...
			for k, v in pairs(usersdata) do
				if (v.uuid == remoteUUID) then
					usersdata[k] = userData;
					goto phoneChannelContinue;
				end
			end

			usersdata[#usersdata + 1] = userData;

			::phoneChannelContinue::
		end
	end

	-- DumpTable(voip.phoneChannels)
...
RegisterNetEvent("TokoVoip:initializeTokovoip");
AddEventHandler("TokoVoip:initializeTokovoip", function(extradata)
	-- Citizen.Trace("TokoVOIP is already running\n")
	...
	for k, v in pairs(extradata) do
		-- print(k, v)
		TokoVoipConfig.plugin_data[k] = v
	end
	...
	-- phone plugin data
	voip.plugin_data.phoneChannel = -1;
	...
	-- phone playerdata setup
	setPlayerData(voip.serverId, "phone:channel", voip.plugin_data.phoneChannel, true);
...
function addPlayerToPhone(channel)
	TriggerServerEvent("TokoVoip:addPlayerToPhone", channel, voip.serverId);
end
RegisterNetEvent("TokoVoip:addPlayerToPhone");
AddEventHandler("TokoVoip:addPlayerToPhone", addPlayerToPhone);

function removePlayerFromPhone(channel)
	TriggerServerEvent("TokoVoip:removePlayerFromPhone", channel, voip.serverId);
end
RegisterNetEvent("TokoVoip:removePlayerFromPhone");
AddEventHandler("TokoVoip:removePlayerFromPhone", removePlayerFromPhone);

RegisterNetEvent("TokoVoip:onPlayerLeavePhoneChannel");
AddEventHandler("TokoVoip:onPlayerLeavePhoneChannel", function(channelId, playerServerId)
	-- Local player left channel
	if (playerServerId == voip.serverId and voip.phoneChannels[channelId]) then
		local previousChannel = voip.plugin_data.phoneChannel;
		voip.phoneChannels[channelId] = nil;
		if (voip.plugin_data.phoneChannel == channelId) then -- If current radio channel is still removed channel, reset to first available channel or none
			if (tablelength(voip.phoneChannels) > 0) then
				for channelId, _ in pairs(voip.phoneChannels) do
					voip.plugin_data.phoneChannel = channelId;
					break;
				end
			else
				voip.plugin_data.phoneChannel = -1; -- No radio channel available
			end
		end

		if (previousChannel ~= voip.plugin_data.phoneChannel) then -- Update network data only if we actually changed radio channel
			setPlayerData(voip.serverId, "phone:channel", voip.plugin_data.phoneChannel, true);
		end

	-- Remote player left channel we are subscribed to
	elseif (voip.phoneChannels[channelId]) then
		voip.phoneChannels[channelId].subscribers[playerServerId] = nil;
	end
end)

RegisterNetEvent("TokoVoip:onPlayerJoinPhoneChannel");
AddEventHandler("TokoVoip:onPlayerJoinPhoneChannel", function(channelId, playerServerId, channelData)
	-- Local player joined channel
	if (playerServerId == voip.serverId and channelData) then
		local previousChannel = voip.plugin_data.phoneChannel;

		voip.plugin_data.phoneChannel = channelData.id;
		voip.phoneChannels[channelData.id] = channelData;
	...
	-- Remote player joined a channel we are subscribed to
	elseif (voip.phoneChannels[channelId]) then
		voip.phoneChannels[channelId].subscribers[playerServerId] = playerServerId;
	end
end)

exports("addPlayerToPhone", addPlayerToPhone);
exports("removePlayerFromPhone", removePlayerFromPhone);
